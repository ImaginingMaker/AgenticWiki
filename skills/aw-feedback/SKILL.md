...
> 深度分析失败根因，生成结构化改进策略，决定回退阶段。
> 即时门禁失败由编排器直接追加到 `prompts.md`，本 Skill 负责**需要回退的严重失败**。

## 触发条件

- 任何阶段门禁失败后用户选择"修复并重试"（编排器已追加即时反馈）
- `aw-validate` 验证失败需要回退
- 用户说"分析根因"、"为什么失败"、"改进策略"
- `aw-orchestrator` 检测到 `currentPhase = FEEDBACK`

---

## 你的任务

1. 读取 `prompts.md` 中最近的失败记录（编排器已追加）
2. 深度分析问题根因（不只是表象，要追问 Why × 3）
3. 生成结构化改进策略（问题 → 根因 → 改进 → 影响技能 → 回退阶段）
4. 决定是否回退到某个阶段
5. 如需回退，清理该阶段及后续产物
