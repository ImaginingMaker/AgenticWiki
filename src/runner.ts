...
function outputGenPrompts(paths: ResolvedPaths): void {
  ...
  // Prompts are generated by gen-scheduler.ts and written to cache/gen-prompts/
  // The gen-schedule.json strips the full prompt (only promptTruncated remains),
  // so we read from the gen-prompts directory instead.
  ...
  for (const entry of toRun) {
    ...
    const genPromptFile = path.join(genPromptsDir, `${entry.id}.md`);

    let promptContent: string;
    try {
      promptContent = fs.readFileSync(genPromptFile, "utf-8");
    } catch {
      promptContent = `[No prompt generated for ${entry.id}]`;
      console.warn(`  ⚠️  未找到 prompt 文件: ${genPromptFile}，使用占位符`);
    }
