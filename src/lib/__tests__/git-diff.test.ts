...
import { getGitDiff, computeAffectedScope } from '../git-diff.js';
...
describe('getGitDiff', () => {
  ...
  it('should detect added files', async () => {
    const mockDiffSummary = vi.fn().mockResolvedValue({
      files: [
        { file: 'src/NewComponent.tsx', insertions: 50, deletions: 0, binary: false, status: 'A' },
      ],
    });
    mockSimpleGit.mockReturnValue({
      diffSummary: mockDiffSummary,
  ...
  it('should detect deleted files', async () => {
    const mockDiffSummary = vi.fn().mockResolvedValue({
      files: [
        { file: 'src/RemovedFile.ts', insertions: 0, deletions: 30, binary: false, status: 'D' },
      ],
    });
    mockSimpleGit.mockReturnValue({
      diffSummary: mockDiffSummary,
  ...
  it('should handle multiple changed files', async () => {
    const mockDiffSummary = vi.fn().mockResolvedValue({
      files: [
        { file: 'src/a.ts', insertions: 1, deletions: 0, binary: false, status: 'A' },
        { file: 'src/b.ts', insertions: 2, deletions: 3, binary: false },
        { file: 'src/c.ts', insertions: 0, deletions: 5, binary: false, status: 'D' },
      ],
    });
    mockSimpleGit.mockReturnValue({
      diffSummary: mockDiffSummary,
  ...
  it('should recognize "added" status string variant', async () => {
    const mockDiffSummary = vi.fn().mockResolvedValue({
      files: [
        { file: 'src/new.ts', insertions: 10, deletions: 0, binary: false, status: 'added' },
      ],
    });
    mockSimpleGit.mockReturnValue({
      diffSummary: mockDiffSummary,
  ...
  it('should recognize "deleted" status string variant', async () => {
    const mockDiffSummary = vi.fn().mockResolvedValue({
      files: [
        { file: 'src/old.ts', insertions: 0, deletions: 10, binary: false, status: 'deleted' },
      ],
    });
    mockSimpleGit.mockReturnValue({
      diffSummary: mockDiffSummary,
  ...
  it('should recursively find transitive dependents', () => {
    ...
    const depGraph: DependencyGraphResult = {
      ...
      modules: [
        ...
        {
          source: 'src/utils.ts',
          dependencies: [{ resolved: 'src/core.ts', type: 'local', circular: false }],
          dependents: ['src/App.tsx'],
          hasCircular: false,
        },
      ],
