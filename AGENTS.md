...
**核心原则**：Agent 运行 `runner.ts`，Runner 自动编排脚本，Agent 只需在 GEN 阶段 spawn SubAgent（经验提取在 GEN 阶段由 SubAgent 并行完成，ASSEMBLE 阶段自动合并）。
...
> 💡 **产物类型选择**：默认产出 Wiki + Issue + 经验三种产物。如需只产出部分类型，使用 `--volumes`：
> ```bash
> npx tsx src/runner.ts --project <path> --volumes wiki,issue      # 只要 Wiki + Issue
...
src/
  ...
  lib/__tests__/          # 41 个测试文件（803 个用例）
...
- **Token 预算 v3**（分段计算：≤10K→2.5x+8K, ≤50K→2.0x+10K, >50K→1.5x+15K，上限 300K，替代旧 80K 硬限制）
...
- **增量模式路径归一化**（git diff 输出的 repo-relative 路径自动剥离 sourceRoot 前缀转为 sourceRoot-relative，与 depGraph 对齐，monorepo 下依赖传播正确生效）
- **resume 状态一致性**（自动重置前重新读取磁盘 state.json，避免陈旧内存覆盖 sync-gen-tasks 刚写入的 completed 状态）
- **assemble-book 聚簇感知**（`assemble-book.ts` CLI 支持 `--clusters`，聚簇模式下章节标签使用 cluster label 而非 folder ID）
- **GEN 验证输出可见**（resume 路径打印 verify-gen-artifacts 的验证详情，失败时用户可见诊断信息）
...
- **通用开发经验提取**（GEN 阶段 SubAgent 并行提取候选经验到 `volume-3-experience/`，ASSEMBLE 阶段 `assemble-experience.ts` 自动合并跨聚簇模式并追加到 `volume-3-experience/`；增量模式自动调用 `computeAffectedExperience` + `markExperienceStale` 标记受影响经验为 stale/orphaned）
- **产物类型选择**（`--volumes wiki,issue,experience` 按需选择产出类型，配置持久化到 `state.json`，`--resume` 和增量模式自动沿用）
- **Issue ↔ Code 双向绑定**（`build-file-issue-index.ts` 生成文件→Issue 反向索引 `file-issues-index.json` + `file-issues.md`；Wiki 页面章节详情表自动注入关联 Issue 链接；`inject-issue-wiki-links.ts` 自动为 Issue 追加 Wiki 页面链接）
- **Experience 独立索引**（`assemble-experience.ts` 生成独立 `experience.md`，含 frontmatter 统计元信息，支持 Obsidian 图谱独立节点）
- **统一入口索引**（`assemble-master-index.ts` 生成 `wiki/index.md` 作为三个 Volume 的唯一入口，聚合 Wiki/Issue/Experience 统计与导航链接）
...
| 增量模式影响范围为 0 | git diff 路径已自动剥离 sourceRoot 前缀与 depGraph 对齐。若仍为 0，检查 `dependency-graph.json` 的 `modules[].source` 是否为 sourceRoot-relative |
...
| `docs/design/experience-extraction.md` | 通用开发经验提取设计文档 |
