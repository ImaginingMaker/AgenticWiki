...
**核心原则**：Agent 运行 `runner.ts`，Runner 自动编排脚本，Agent 只需在 GEN 阶段 spawn SubAgent（经验提取为 ASSEMBLE 内可选步骤，也可 spawn SubAgent）。
...
src/
  ...
  lib/__tests__/          # 38 个测试文件（768 个用例）
...
- **通用开发经验提取**（ASSEMBLE 阶段自动运行 `extract-experience.ts` 和 `assemble-experience.ts`，跨聚簇提取通用实现模式到 `volume-3-experience/`；增量模式自动调用 `computeAffectedExperience` + `markExperienceStale` 标记受影响经验为 stale/orphaned）
