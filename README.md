...
7. **标记受影响 Issue**：自动将源文件变更的 Issue 标记为 stale
8. **标记受影响经验**：自动检测跨聚簇经验模式，标记为 stale（源聚簇变更但仍有 ≥2 个有效源）或 orphaned（源聚簇 <2）
9. **暂停**：输出 prompts，等待 Agent spawn SubAgent
...
| `--limit N` | `number` | | `5` | GEN 阶段每批调度 N 个子任务。与 `--token-limit` 互斥（token-limit 优先） |
...
| SubAgent 模板读取失败 | v3 已移除模板外置机制，规则直接内联到 SubAgent prompt 中 |
...
```
Agent（读本文件）→ runner.ts（自动编排 6 阶段）→ 35 个脚本
                                       ↘ GEN 暂停 → Agent spawn SubAgent
                                       ↘ ASSEMBLE 内可选经验提取 → Agent spawn SubAgent（可选）
```
...
| ASSEMBLE | 符号索引 + Issue 去重 + 仪表盘 + 组装成书 + 经验提取 | ❌ |
...
| 优化 | 说明 | 收益 |
|:---|:---|:---:|
| **Token 预算 v3** | 预算按任务大小分段计算（≤10K:2.5x+8K, ≤50K:2.0x+10K, >50K:1.5x+15K），上限 200K | 大任务不再被 80K 硬限制 |
...
| **动态拆分阈值** | 50K/30K/5K → 120K/15K 基于项目总 Token × 百分比动态计算 | 适配 1M 上下文模型 |
| **入口文件内联** | 纯 re-export 的 `index.ts` 自动合并到相邻 subTask，不单独生成 | 减少无效 subTask，节省 Token |
| **文件元信息提取** | `extract-file-meta.ts` 预分析组件/Hook/Props/export，SubAgent 读取摘要而非源码（全量读取，无 4KB 截断） | SubAgent Token 减少 ~60% |
| **依赖聚簇划分** | `cluster-tasks.ts` 按依赖 BFS 聚簇替代文件夹+角色划分 | subTask 数量减少 50-60% |
| **FileTaskIndex 双向索引** | `build-file-task-index.ts` 构建文件↔任务映射，增量模式同时支持聚簇和文件夹策略 | 增量模式不再硬编码文件夹策略 |
| **Issue 去重** | `dedup-issues.ts` 按 (type, source_files) 精确匹配，自动归档重复 Issue | 避免多 SubAgent 产生语义重复 |
| **Issue 状态机** | `issue-status.ts` 激活 11 种状态流转（detected→verified/disputed/stale/duplicate），验证自动更新状态 | 状态不再是死代码 |
| **非关键阶段标记** | `validate-references.ts` 标记为 non-critical，sourceFiles 缺失不阻塞流水线 | 避免 VALIDATE 阶段误卡流水线 |
| **进度面板聚簇感知** | `progress-dashboard.ts` 优先从 `state.genTasks` 构建仪表盘 | 聚簇模式正确显示 100% |
| **SubAgent 产物自检** | SubAgent prompt 内置步骤 3.5，写入后用 `ls -la` 验证文件存在且非空 | 减少 SubAgent 静默失败 |
| **通用开发经验提取** | ASSEMBLE 阶段自动提取跨聚簇通用实现模式，生成 `volume-3-experience/` 经验知识库；增量模式自动标记 stale/orphaned | 加速新人上手，复用成熟方案 |
